...
object OptionVsNPE extends App {
  def getConnection(resource: Resource): Connection = Option(ConnectionProducer.produce(resource)) match {
    case Some(connection) => connection
    case _ => getConnection(resource)
  }

  def businessLogic: String = try {
    // ResourceProducer
    val result: String = Option(ResourceProducer.produce) match {
      case Some(resource) => getConnection(resource).result()
    ...
    case _: ResourceException => println("Try again with new resource")
      businessLogic
  }
